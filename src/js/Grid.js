...
function Grid(){
	...
	this.setup = function(teams, id, subclass){
		...
		if (subclass == undefined){
			...
			// this.animations.setup(teams, id)
		}
		this.createNodes();
	}
	...
	this.start = function(){
		...
		this.setHomepageNodeGraph();
		this.drawRectangles();
		// this.animations.start('interference')
	}
	...
	*/
		for (let i in this.rects){
			// if ( window._nodes.getNodeLocationReal(i) == 50 ) {
			// 	var im = document.getElementById("PatternRight")
			// 	this.elem.drawImage(im, 0, 0, 1000, 961, this.rects[i].x-50, this.rects[i].y-40, 1000/8, 961/8)
			// }
			this.elem.beginPath();
			...
			this.elem.fillText(window._nodes.getNodeLocationReal(i),this.rects[i].x,this.rects[i].y+10);
		}
	...
	this.on = function(node, color, alpha){
		...
		this.elem.font = "10px Arial";
		this.elem.fillStyle = "#2c3e50";
		this.elem.fillText(window._nodes.getNodeLocationReal(node),this.rects[node].x,this.rects[node].y+10);
	...
	this.fadeIn = function(node, team, color, last_node){
		...
		if ( this.rects[node].alpha < 1.0) {
			...
			new_node = window._nodes.getUntakenNode(node);
			if ( new_node == null && last_node != null) {
				new_node = window._nodes.getUntakenNode(last_node);
				if ( new_node != null ) {
					window._nodes.takeNode(new_node)
					this.teams[team].team.setRadio({radioDirection: {value: window._nodes.getNodeDirection(new_node, last_node), type: "imgSrc"}});
					if (this.autoplay){
				...
				window._nodes.takeNode(new_node);
				this.teams[team].team.setRadio({radioDirection: {value: window._nodes.getNodeDirection(new_node, node), type: "imgSrc"}});
				if (this.autoplay){
	...
	this.startInterference = function(node, team, color, last_node){
		...
		if ( this.rects[node].taken == 0 ) {
			...
			console.log('ALREADY TAKEN!', node, color, this.rects)
			var new_nodes = window._nodes.getSurroundingNodes(node)
			if (this.autoplay){
				for ( i in new_nodes) {
					if ( new_nodes[i] != -1 && $.inArray(new_nodes[i], window._nodes.getTakenNodes()) == -1 ){
	...
	*/
	...
	this.setHomepageNodeGraph = function(){
		for (let i=0; i<15; i++){
			$('#node_'+i).closest('tr').find('.text').text('Node '+window._nodes.getNodeLocationReal(i));
		}
	}
}